...
export type MovieProps = {
  ...
  hasGenre?: boolean;
  genre?: string;
  onSelectMovie?: (id: number, movie: Movie) => void; // Accept function prop
};
export const MovieCard = ({
  ...
  return (
    <div
      ...
      style={{ zIndex }}
      onClick={() => onSelectMovie?.(movie.id, movie)}
    >
      <img
        loading={"lazy"}
      ...
      />
      <div className="invisible absolute top-0 w-full scale-0 opacity-0 transition delay-100 duration-200 group-hover:-translate-y-[3vw] group-hover:translate-x-[1vw] group-hover:scale-120 group-hover:opacity-100 group-hover:shadow-xl sm:visible">
        <img
          loading={"lazy"}
          src={`https://image.tmdb.org/t/p/original/${movie.poster_path}`}
          alt={movie.title}
        ...
        <div className="shadow-ms absolute z-10 w-full rounded-b-md bg-zinc-800 p-2 transition">
          ...
          <div className="flex flex-col items-start gap-2">
            <p className="text-[10px] text-white lg:text-sm">
              2h 30m{" "}
              {hasGenre && (
                <span className="text-[10px] text-white lg:text-sm">
                  {"~"} {genre}
                </span>
              )}
            </p>
